...
# Rscript Fig5a.1.SCtoBeta.Plot.r
...
source_data_dir <- normalizePath(file.path(script_dir, "..", "..", "SourceData", "Fig5a.1.SCtoBeta"), mustWork = FALSE)
selected_up_file <- file.path(source_data_dir, "Selected.Up.csv")
selected_dn_file <- file.path(source_data_dir, "Selected.DN.csv")
...
Decreased.Signal <- c(
  "RA-ALDH1A1 - (RXRG+CRABP2)",
  "RA-ALDH1A3 - (RXRG+CRABP2)",
  "RA-ALDH1A3 - (RXRA+CRABP2)",
  "LAMA4 - SV2A"
)
...
ggsave(
  filename = "Fig5a.1.SCtoBeta.Plot.pdf",
  plot = p +
    theme(
...
ggsave(
  filename = "Fig5a.1.SCtoBeta.Plot.right.pdf",
  plot = p_right +
    theme(
