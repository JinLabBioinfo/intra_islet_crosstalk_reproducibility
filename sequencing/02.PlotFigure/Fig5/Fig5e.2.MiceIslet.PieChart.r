...
# Rscript Fig5e.2.MiceIslet.PieChart.r
...
source_data_dir <- normalizePath(file.path(script_dir, "..", "..", "SourceData", "Fig5e.2.MiceIslet.PieChart"), mustWork = FALSE)
input_file <- file.path(source_data_dir, "MiceIslet_CellTypeNew_MetaData.csv")

setwd(script_dir)

meta <- read.csv(input_file, stringsAsFactors = FALSE)

required_cols <- c("FinalCluster", "CellTypeNew")
...
p <- ggplot(celltype_df) +
  ...
  labs(title = "Mice Islet Cell Type Composition", fill = "Cell Type") +
  theme_void() +
  theme(
...
ggsave("Fig5e.2.MiceIslet.PieChart.pdf", p, width = 7, height = 6)
ggsave("Fig5e.2.MiceIslet.PieChart.png", p, width = 7, height = 6, dpi = 300)
