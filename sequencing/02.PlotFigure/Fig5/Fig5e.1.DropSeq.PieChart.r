...
# Rscript Fig5e.1.DropSeq.PieChart.r
...
source_data_dir <- normalizePath(file.path(script_dir, "..", "..", "SourceData", "Fig5e.1.DropSeq.PieChart"), mustWork = FALSE)
input_file <- file.path(source_data_dir, "DropSeq.Meta.Data.csv")

setwd(script_dir)

Metadata <- read.csv(input_file, stringsAsFactors = FALSE)

required_cols <- c("CellType", "CellTypeNew")
...
celltype_counts <- table(Metadata$CellTypeNew)
...
explode_dist <- 0.00
celltype_df$shift <- ifelse(celltype_df$CellType == "SC", explode_dist, 0)
celltype_df$x0 <- celltype_df$shift * cos(celltype_df$mid)
celltype_df$y0 <- celltype_df$shift * sin(celltype_df$mid)
...
p <- ggplot(celltype_df) +
  ...
  labs(title = "DropSeq Cell Type Composition", fill = "Cell Type") +
  theme_void() +
  theme(
...
ggsave("Fig5e.1.DropSeq.PieChart.pdf", p, width = 7, height = 6)
ggsave("Fig5e.1.DropSeq.PieChart.png", p, width = 7, height = 6, dpi = 300)
