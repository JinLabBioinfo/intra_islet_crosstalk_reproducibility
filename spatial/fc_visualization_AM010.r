...
geo_adj <- return_geoadj(readRDS("for_jasmine/c2_list_AM010.rds"))
...
write.csv(
  ...
  "for_jasmine/geo_adjacency_AM010.csv",
  quote = FALSE
)

# Load annotated and raw Seurat objects for AM010
obj_merged <- LoadSeuratRds("for_jasmine/obj_merged_AM010_annotated.rds")
obj_raw <- LoadSeuratRds("for_jasmine/obj_raw_AM010.rds")

# UMAP of all cells; endocrine in gold, nonendocrine in royalblue (simplified palette)
ggsave("for_jasmine/umap_all_AM010.png",
  DimPlot(obj_merged,
    dims = c(2, 1),
    reduction = "umap",
    shuffle = TRUE,
...
ggsave("for_jasmine/umap_nonendocrine_AM010.png",
  DimPlot(
    quick_reanalyze_subset(obj_merged, c("aStellate", "qStellate", "Ductal", "Acinar", "Immune", "Inconclusive")),
    dims = c(2, 1),
    reduction = "umap",
    shuffle = TRUE,
    raster = FALSE,
...
ggsave("for_jasmine/dotplot_marker_AM010.pdf",
  DotPlot(obj_merged,
    features = c(Alpha, Beta, Delta, Gamma, aStellate, qStellate, Ductal, Acinar, Immune),
    group.by = "cell_type",
...
ggsave("for_jasmine/image_full_AM010.png",
...
ggsave("for_jasmine/image_fov_AM010.png",
  ImageDimPlot(
    subset(obj_raw,
      subset = fov %in% c(31, 32, 33, 44, 45, 46, 68, 69, 70)
    ),
...
obj_stellate <- process_stellate(obj_menv, obj_exo, extract_barcodes(readRDS("for_jasmine/c1_list_AM010.rds")))
...
dual_logfc_barplot(sig_dropseq, sig_cosmx, azimuth_stellate_sig_list, "for_jasmine/barplot_log2fc_AM010.pdf")
