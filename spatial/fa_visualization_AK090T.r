...
geo_adj <- return_geoadj(readRDS("for_jasmine/c2_list_AK090T.rds"))
...
write.csv(
  ...
  "for_jasmine/geo_adjacency_AK090T.csv",
  quote = FALSE
)

# Load annotated and raw Seurat objects for AK090T
obj_merged <- LoadSeuratRds("for_jasmine/obj_merged_AK090T_annotated.rds")
obj_raw <- LoadSeuratRds("for_jasmine/obj_raw_AK090T.rds")

# UMAP of all cells; endocrine in gold, nonendocrine in royalblue (simplified palette)
ggsave("for_jasmine/umap_all_AK090T.png",
  DimPlot(obj_merged,
    dims = c(2, 1),
    reduction = "umap",
    shuffle = TRUE,
...
ggsave("for_jasmine/umap_nonendocrine_AK090T.png",
  DimPlot(
    quick_reanalyze_subset(obj_merged, c("aStellate", "qStellate", "Ductal", "Acinar", "Inconclusive")),
    dims = c(2, 1),
    reduction = "umap",
    shuffle = TRUE,
    raster = FALSE,
    cols = c(
      ...
      "#006400", # Ductal
      "#A020F0", # Acinar
      "#5A1FF0" # Inconclusive (named as Acinar/Ductal in the publication)
    )
  ) & theme(
    axis.line = element_blank(),
    axis.text = element_blank(),
...
ggsave("for_jasmine/dotplot_marker_AK090T.pdf",
  DotPlot(obj_merged,
    features = c(Alpha, Beta, Delta, Gamma, aStellate, qStellate, Ductal, Acinar),
    group.by = "cell_type",
...
ggsave("for_jasmine/image_full_AK090T.png",
...
ggsave("for_jasmine/image_fov_AK090T.png",
  ImageDimPlot(
    subset(obj_raw,
      subset = fov %in% c(441, 440, 439, 418, 417, 416, 395, 394, 393)
    ),
...
obj_stellate <- process_stellate(obj_menv, obj_exo, extract_barcodes(readRDS("for_jasmine/c1_list_AK090T.rds")))
...
dual_logfc_barplot(sig_dropseq, sig_cosmx, azimuth_stellate_sig_list, "for_jasmine/barplot_log2fc_AK090T.pdf")
