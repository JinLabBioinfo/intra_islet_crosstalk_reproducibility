...
# Rscript Fig6c.2.RetinoicAcidDerivatives.r
...
source_data_dir <- normalizePath(file.path(script_dir, "..", "..", "SourceData", "Fig6c.2.RetinoicAcidDerivatives"), mustWork = FALSE)
setwd(script_dir)

ListData <- c(
  "RetinoicAcid" = file.path(source_data_dir, "01.RetinoicAcid.csv"),
  "13-cis-RetinoicAcid" = file.path(source_data_dir, "02.13-cis-RetinoicAcid.csv"),
  "All-trans-4-oxoretinoicAcid" = file.path(source_data_dir, "03.All-trans-4-oxoretinoicAcid.csv"),
  "all-trans-5-6-EpoxyretinoicAcid" = file.path(source_data_dir, "04.all-trans-5-6-EpoxyretinoicAcid.csv"),
  "all-trans-retinoyl-beta-glucuronide" = file.path(source_data_dir, "05.all-trans-retinoyl-beta-glucuronide.csv")
)
...
for (met_name in names(mat_list)) {
  ra_mat <- mat_list[[met_name]]

  ra_df <- as.data.frame(ra_mat) %>%
  ...
  for (cell_type in OrderCellType) {
    ...
    ggsave(
      file.path(script_dir, paste0("Fig6c.2.", safe_name, "_", cell_type, "_barplot.pdf")),
